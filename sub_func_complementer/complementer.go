...
import (
	...
	"data-platform-api-planned-train-operation-creates-rmq-kube/config"
	"encoding/json"
...
func (c *SubFuncComplementer) ComplementHeader(input *dpfm_api_input_reader.SDC, subfuncSDC *SDC, l *logger.Logger) error {
	s := &SDC{}
	numRange, err := c.ComplementPlannedTrainOperation(input, l)
	if err != nil {
		return xerrors.Errorf("complement plannedTrainOperation error: %w", err)
	}
...
func (c *SubFuncComplementer) ComplementPlannedTrainOperation(input *dpfm_api_input_reader.SDC, l *logger.Logger) (*NumberRange, error) {
	rows, err := c.db.Query(
		...
		WHERE (ServiceLabel, FieldNameWithNumberRange) = ( (?, 'PlannedTrainOperationID') );`, input.ServiceLabel,
	)
	if err != nil {
	...
	nr.LatestNumber++
	input.Header.PlannedTrainOperation = nr.LatestNumber
	return &nr, nil
}
